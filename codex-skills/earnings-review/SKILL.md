...
如果无法获取完整原文，按 `skills/financial-data.md` 规范使用标准数据源拼凑（美股/港股/A股：主源 腾讯自选股-金融数据查询 `westockdata` + 副源 NeoData金融搜索服务 `neodata-financial-search`，后备：东方财富妙想金融数据 `mx-finance-data`；台股：FinMind `tools/twstock_data.py`+Goodinfo），但必须标注"非原始财报，来自第三方汇总"，且关键数据两源误差>1%须标记。
