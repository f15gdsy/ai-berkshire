...
如果无法获取完整原文，按 `skills/financial-data.md` 规范使用标准数据源拼凑（美股/港股/A股：主源 腾讯自选股-金融数据查询 `westockdata` + 横向对照源 通达信 MCP `tdx-connector`；通达信不可用时降级用副源 NeoData金融搜索服务 `neodata-financial-search` 验证；后备：东方财富妙想金融数据 `mx-finance-data`。台股：FinMind `tools/twstock_data.py`+Goodinfo），但必须标注"非原始财报，来自第三方汇总"，且关键数据两源误差>1%须标记。
...
必须覆盖：
- 总收入及分业务/分地区收入拆解 → 拆解明细呈现于报告第三节"各业务线数据与运营情况"专章
- 毛利润、毛利率变化
...
- [ ] **分部信息**：不同业务的利润率差异，是否有"好业务补贴坏业务"——分部利润率差异同时进入第三节专章的"指引 vs 实际"与 Σ校验
...
**第三节"各业务线数据与运营情况"的内容规范**（公司归根结底通过各业务线支撑业绩）：

> **数据来源规则**：分部数据以原始财报分部附注为主源（美股 10-K segment footnote、港股年报"分部资料"、A股"分部收入附注"）——本 skill 以一手资料为纲，此节天然以分部附注为据。二手源通常无分部颗粒度；无法二手交叉验证时允许单源，但必须标注「单源（分部数据）·来源：{财报名称+附注编号}」。
...
各业务线逐条（每条 ≤10 行）：
- **量价拆解 + 运营指标**：销量/单价、GMV、订单量、MAU/DAU、ARPU、翻台率等——按行业适配选 2-4 个最相关的
- **驱动因素**：增长/下滑归因（量、价、结构、新业务）
- **指引 vs 实际**：管理层上期对该业务线的指引 vs 本期实际（✅/⚠️/❌）

**Σ校验（必须执行）**：Σ各业务线收入 ≈ 总收入，用 `financial_rigor.py cross-validate` 抽验；偏差>1%须排查口径（内部抵销/未分摊项）。

**降级规则**：B/C 级资料可得性时，本节允许降级为分部总览表 + 「数据不足」标注；禁止推算填充。
