...
- description 包含：
  1. 商业模式本质：核心生意定义、收入结构拆解；**从财报分部附注收集各业务线收入/增速/毛利率、量价拆解与行业适配运营指标（GMV/订单量/MAU/DAU/ARPU 等，按行业选 2-4 个最相关的），二手源无分部颗粒度时标注「单源（分部数据）·来源」**
  2. 平台/产品飞轮效应如何运转
...
- description 包含：
  ...
  7. **分部 Σ校验（必须执行）**：Σ各业务线收入 ≈ 总收入，用 `financial_rigor.py cross-validate` 验证；偏差>1%须排查口径（内部抵销/未分摊项）
...
- description 包含：
  ...
  6. 产业链分析：上中下游价值分配
  7. 各业务线运营指标的行业对标：与同行同口径对比（GMV/单量/MAU/ARPU 等），判断各业务线是跑赢还是跑输行业
  8. 要求搜索最新行业数据和竞争动态
...
- **财务数据必须来自两个独立来源**，按 `skills/financial-data.md` 规范执行（美股/港股/A股：主源 腾讯自选股-金融数据查询 `westockdata` + 横向对照源 通达信 MCP `tdx-connector`；通达信不可用时降级用副源 NeoData金融搜索服务 `neodata-financial-search` 验证；后备：东方财富妙想金融数据 `mx-finance-data`。台股：FinMind `tools/twstock_data.py`+Goodinfo），两源误差>1%须标记
- 分部数据（各业务线收入/利润率/运营指标）二手源通常无颗粒度，允许单源但须标注「单源（分部数据）·来源：{财报名称+附注编号}」
- 确保数据准确，关键数据标注来源
- 分析要深入，不流于表面
...
> 本节由 business-analyst 的分部材料为主、financial-analyst 的 Σ校验与分部利润率补充，team-lead 去重合并而成。

> **数据来源规则**：分部数据以原始财报分部附注为主源（美股 10-K segment footnote、港股年报"分部资料"、A股"分部收入附注"）。二手源通常无分部颗粒度；无法二手交叉验证时允许单源，但必须标注「单源（分部数据）·来源：{财报名称+附注编号}」。

分部总览表：

| 业务线 | 收入 | 同比 | 环比/占比 | 毛利率或经营利润率 | 利润率同比变动 |
|--------|------|------|----------|------------------|---------------|

各业务线逐条（每条 ≤10 行）：
- **量价拆解 + 运营指标**：按行业适配选 2-4 个最相关的（GMV、订单量、MAU/DAU、ARPU、翻台率等），附行业对标（来自 industry-researcher）
- **驱动因素**：增长/下滑归因（量、价、结构、新业务）
- **指引 vs 实际**：管理层上期指引 vs 本期实际（✅/⚠️/❌）

留白原则：无分部附注或指标缺失 → 标「数据不足」，禁止推算填充。
