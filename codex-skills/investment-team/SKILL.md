...
- **财务数据必须来自两个独立来源**，按 `skills/financial-data.md` 规范执行（美股/港股/A股：主源 腾讯自选股-金融数据查询 `westockdata` + 副源 NeoData金融搜索服务 `neodata-financial-search`，后备：东方财富妙想金融数据 `mx-finance-data`；台股：FinMind `tools/twstock_data.py`+Goodinfo），两源误差>1%须标记
- 确保数据准确，关键数据标注来源
- 分析要深入，不流于表面
