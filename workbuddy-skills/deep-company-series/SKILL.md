...
description: "看懂XX公司（深度公司系列）：为一家公司撰写 3-8 篇深度长文系列（按公司复杂度定篇数），发布公众号/视频号。核心 IP 是\\"会改\\"——内置 99% 财经文章都在违反的事实核查标准。触发词：看懂XX、深度公司系列、公司长文系列、拆一家公司。输入：公司名（如\\"腾讯\\"）。"
---

## Harness adapter note

本 skill 由 `skills/deep-company-series.md` 生成，可在 WorkBuddy 及任何支持 Agent Skills 规范的 harness 中独立运行。
...
参考样本（可选）：`reports/腾讯/《看懂腾讯》/`——仅在 ai-berkshire 仓库内运行时可用；独立环境跳过样本参照，按本 skill 内置标准执行。
...
**不适合用本 skill 的场景**：单篇研报、季报点评、行业研究——那些用 `investment-research`、`earnings-review`、`industry-research`（skill 名，未安装时独立降级执行相应部分）。
...
3. 用 `investment-team` 或 `investment-research`（skill 名，未安装时独立降级执行）先生成内部研究底稿
