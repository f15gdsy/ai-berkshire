---
name: earnings-team
description: "财报精读团队：四大师（段永平看生意本质、巴菲特审财务质量、芒格读竞争变化、李录猎风险信号）并行解读 + 编辑润色 + 读者评审，产出研究报告与可直接发布的公众号文章。适合重要公司的关键财报。触发词：财报精读团队、四大师财报、团队精读、财报公众号文章。输入：`公司名 季度`（如 `腾讯 2025Q4`）。"
---

## Harness adapter note

本 skill 由 `skills/earnings-team.md` 生成，可在 WorkBuddy 及任何支持 Agent Skills 规范的 harness 中独立运行。
...
**本 Agent 是最终底稿"各业务线数据与运营情况"专章的主供方**——分析内容 1、2 的产出将直接合成该专章。
...
1. **收入结构拆解与解读**
   ...
   - 收入增长来自"量"还是"价"？哪种更健康？
   - **分部数据规范**：以原始财报分部附注为主源（10-K segment footnote / 港股年报"分部资料" / A股"分部收入附注"）；二手源无分部颗粒度时允许单源，须标注「单源（分部数据）·来源：{财报名称+附注编号}」；无分部附注 → 标「数据不足」，禁止推算填充

2. **用户/客户价值变化**
   - DAU/MAU/付费用户等运营指标变化——**按行业适配选取 2-4 个最相关的**（如电商看 GMV/订单量、外卖看配送单量、游戏看流水/递延收入、本地生活看翻台率/单量）
   - 用户时长、ARPU、留存率等质量指标
   - 平台/产品对用户的价值在增强还是减弱？
...
5. **管理层产品直觉**
   ...
   - **分部"指引 vs 实际"**：管理层上期对各业务线的指引 vs 本期实际（✅/⚠️/❌）
...
1. **核心财务数据提取与验证**
   ...
   - 关键数据至少两个来源交叉验证
   - **分部 Σ校验**：Σ各业务线收入 ≈ 总收入，用 `financial_rigor.py cross-validate` 验证；偏差>1%须排查口径（内部抵销/未分摊项）；同时提供各业务线毛利率/经营利润率供专章使用
...
> 本节由 Agent1（段永平）的收入拆解与运营指标材料为主合成，Agent2（巴菲特）提供 Σ校验与分部利润率，Team Lead 去重合并。
...
各业务线逐条（每条 ≤10 行）：
- **量价拆解 + 运营指标**：按行业适配选 2-4 个最相关的（GMV、订单量、MAU/DAU、ARPU、翻台率等）
...
> 以上均为 skill 名（非斜杠命令）。相关 skill 未安装时，独立降级执行相应部分。
