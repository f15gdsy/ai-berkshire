...
description: "组合管理审视：从\\"研究公司\\"升级到\\"管理组合\\"——对持仓组合执行审视与优化（集中度、相关性、每只持仓的论文健康度、加减仓建议）。触发词：组合审视、持仓检查、组合优化、我的持仓怎么办。输入：持仓清单（如 `腾讯30%, 美团20%, 现金15%`）或 `我的持仓`（读取已保存组合）。"
---

## Harness adapter note

本 skill 由 `skills/portfolio-review.md` 生成，可在 WorkBuddy 及任何支持 Agent Skills 规范的 harness 中独立运行。
...
如果组合中有"不如现金"的仓位，或者现金占比过高，建议使用 `industry-research` 或 `investment-checklist`（skill 名，未安装时独立降级执行）对感兴趣的行业/公司进行系统筛选，而非在本Skill内直接推荐个股。
