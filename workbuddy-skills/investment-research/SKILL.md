...
description: "投资研究（四大师综合分析）：基于巴菲特、芒格、段永平、李录四位投资大师的方法论，按八个模块顺序对一家公司执行系统化投资研究。触发词：投资研究、深度研究一家公司、四大师分析、公司基本面研究。输入：公司名。"
---

## Harness adapter note

本 skill 由 `skills/investment-research.md` 生成，可在 WorkBuddy 及任何支持 Agent Skills 规范的 harness 中独立运行。
...
基于巴菲特、芒格、段永平、李录四位投资大师的方法论，按以下八个模块顺序执行研究：
...
1. 收入结构：最近财年及近4季度分部收入、增速、毛利率 → 明细呈现于第三步专章
...
分析要点：
- 用一句话定义这门生意的本质
- 收入结构：见第三步专章（数据与运营事实层）；此处只做生意本质解读，不重复表格
- 5年盈利能力趋势（图表）
...
> 本节是数据与运营事实层，替代原"收入结构拆解"的表格部分；第二步保留商业逻辑解读，二者不重复。公司归根结底通过各业务线支撑业绩，清晰地理解业务表现是投研的重中之重。

> **数据来源规则**：分部数据以原始财报分部附注为主源（美股 10-K segment footnote、港股年报"分部资料"、A股"分部收入附注"）。westockdata/NeoData/妙想/通达信等二手源通常无分部颗粒度；无法二手交叉验证时允许单源，但必须标注「单源（分部数据）·来源：{财报名称+附注编号}」。
...
各业务线逐条分析（每条 ≤10 行）：
- **量价拆解 + 运营指标**：销量/单价、GMV、订单量、MAU/DAU、ARPU、翻台率、配送单量等——按行业适配选 2-4 个最相关的
- **驱动因素**：增长/下滑归因（量、价、结构、新业务）
- **指引 vs 实际**：管理层上期指引 vs 本期实际（✅/⚠️/❌）

**Σ校验（必须执行，使用金融严谨性工具）**：Σ各业务线收入 ≈ 总收入，用 `financial_rigor.py cross-validate` 抽验；偏差>1%须排查口径（内部抵销/未分摊项）。
...
**这一步不能跳过的原因**：第八步的 audit 是在算数之前做的，而写报告的过程中经常会回头调 g 或 r。**准出前的最后一次 audit 才是对报告负责的那一次。**
