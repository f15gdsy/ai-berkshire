...
description: "财报精读（单Agent）：基于一手资料（财报原文、电话会纪要、股东信）对单份财报做深度解读，\\"我从不看卖方研报，只读原始财报\\"。轻量版，适合快速过一遍。触发词：财报精读、解读财报、财报分析、季报点评。输入：`公司名 季度`（如 `腾讯 2025Q4`、`PDD 2025年报`，默认最近一期）。"
---

## Harness adapter note

本 skill 由 `skills/earnings-review.md` 生成，可在 WorkBuddy 及任何支持 Agent Skills 规范的 harness 中独立运行。
