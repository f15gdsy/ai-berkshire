...
**AI Berkshire** is a collection of investment research skills compatible with Claude Code, Codex, and WorkBuddy. It systematizes the methodologies of four value investing masters — Buffett, Munger, Duan Yongping, and Li Lu — and delivers professional-grade research through AI Agents.
...
This repository keeps one canonical workflow and provides Claude Code commands, Codex skills, and WorkBuddy skills. Install the client you plan to use.
...
For WorkBuddy users on macOS / Linux:

```bash
# Clone the repository
git clone https://github.com/xbtlin/ai-berkshire.git

# Generate and install WorkBuddy skills to ~/.workbuddy/skills
cd ai-berkshire
./scripts/install-workbuddy-skills.sh
```

For WorkBuddy users on Windows PowerShell / Command Prompt:
...
After installing, restart WorkBuddy (or refresh its skill list). Note: `ai-berkshire-tools` is a shared dependency of the other AI Berkshire WorkBuddy skills (it carries copies of validation scripts such as `financial_rigor.py`) and must be installed alongside them — the install script handles this automatically.

The repository maintains four entry points: `skills/*.md` are the Claude Code command sources (with YAML frontmatter; the trigger words in `description` drive automatic skill matching across clients); `codex-skills/*/SKILL.md` are Codex skill packages generated from `skills/*.md` by `scripts/sync-codex-skills.py`; `codex-prompts/*.md` are an optional Codex slash-prompt compatibility layer; `workbuddy-skills/*/SKILL.md` are WorkBuddy skill packages (including the shared `ai-berkshire-tools` package) generated by `scripts/sync-workbuddy-skills.py` — they can also be copied standalone into any client that supports the Agent Skills convention (`<name>/SKILL.md` + frontmatter).
...
In WorkBuddy, restart after installing and describe tasks in natural language — skills are matched automatically via the trigger words in their descriptions, for example:

```text
Use the earnings team skill to analyze Tencent 2025Q4
Screen the AI compute industry for investment targets
Answer in Duan Yongping's style: where is Pinduoduo's real moat?
Write a WeChat article about Meituan
```
